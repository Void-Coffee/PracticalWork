...
public class PlayerController : GridEntity
{
    ...
    void TryMove(Vector2Int direction)
    {
        ...
        if (hit == null)
        {
            // Nothing in the way - save the "before" state, then walk.
            if (UndoManager.Instance != null)
                UndoManager.Instance.SaveSnapshot();
        ...
        if (box != null)
        {
            // Save BEFORE attempting the push, since the box may start moving
            // the instant TryPush is called.
            if (UndoManager.Instance != null)
                UndoManager.Instance.SaveSnapshot();
